...
var TeXLive = function (opt_workerPath) {
  ...
  var component = function (workerPath) {
    ...
    self.on_stdout = function (msg) {
      console.log(msg);
      $output.innerHTML +=
        "<small><code>" + msg + "</code></small><hr class='my-1'>";
    };
  ...
  pdftex.compileRaw = function (source_code) {
    var self = this;
    return pdftex.run(source_code).then(function () {
  ...
  pdftex.run = function (source_code) {
    ...
    return promise.chain(commands).then(sendCompile);
  };
  ...
  bibtex.compile = function (aux) {
    var self = this;
    var p = new promise.Promise();
    bibtex.compileRaw(aux).then(function (binary_bbl) {
      if (binary_bbl === false) return p.done(false);
      pdf_dataurl =
        "data:text/plain;charset=binary;base64," + window.btoa(binary_bbl);
      return p.done(pdf_dataurl);
    });
    return p;
  };
  bibtex.compileRaw = function (aux) {
    var self = this;
    return bibtex.run(aux).then(function () {
